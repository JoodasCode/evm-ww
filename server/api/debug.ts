...
/**
 * Force insert a test wallet profile directly into Supabase
 * POST /api/debug/force-insert
 * Body: { walletAddress }
 */
router.post('/force-insert', async (req, res) => {
  try {
    ...
    console.log('[DEBUG] Force inserting wallet profile for:', walletAddress);
    
    // 1. Log Supabase connection details
    ...
    console.log('[DEBUG] Using normalized address:', normalizedAddress);
    ...
    const testProfile = {
      ...
      display_name: `Debug Test ${new Date().toISOString()}`,
      verification_signature: 'debug_test'
    };
    
    console.log('[DEBUG] Attempting direct Supabase insert...');
    ...
    if (directError) {
      console.error('[DEBUG] Direct insert error:', directError);
    ...
    console.log('[DEBUG] Direct insert successful:', directInsert);
    
    // 3. Try service method
    console.log('[DEBUG] Attempting insert via WalletProfileService...');
    const walletProfileService = WalletProfileService.getInstance();
    const serviceResult = await walletProfileService.createOrUpdateWalletProfile(
      walletAddress,
      'evm'
    );
    
    console.log('[DEBUG] Service method result:', serviceResult);
    
    // 4. Verify the profile exists
    console.log('[DEBUG] Verifying profile exists...');
    ...
    if (verifyError) {
      ...
      return res.status(500).json({
        success: false,
        error: 'Failed to verify wallet profile exists',
        details: verifyError
      });
    }
    ...
    console.error('[DEBUG] Unexpected error in force-insert:', error);
    return res.status(500).json({
      success: false,
...
/**
 * Check if a wallet profile exists in Supabase
 * GET /api/debug/check-wallet/:address
 */
router.get('/check-wallet/:address', async (req, res) => {
  try {
    ...
    console.log('[DEBUG] Checking wallet profile for:', normalizedAddress);
    ...
    if (error && error.code !== 'PGRST116') {
      console.error('[DEBUG] Error checking wallet profile:', error);
      return res.status(500).json({
        success: false,
        error: 'Failed to check wallet profile',
        details: error
      });
    }
    ...
    console.error('[DEBUG] Unexpected error in check-wallet:', error);
    return res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error occurred'
    });
 ...
 */
// Define step interface to fix TypeScript errors
interface DebugStep {
  step: number;
  name: string;
  status: string;
