...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'wallet_address_unique' AND table_name = 'wallet_profiles'
  ) THEN
...
DROP TRIGGER IF EXISTS update_wallet_profile_last_updated_trigger ON wallet_profiles;
...
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_profiles') THEN
    -- First drop the RLS policy that depends on user_id
    DROP POLICY IF EXISTS "Users can view their own wallet profiles" ON wallet_profiles;
