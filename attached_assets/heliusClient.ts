...
/**
 * Fetches enriched transaction details
 * @param transactionSignature - The transaction signature
 * @returns Enriched transaction data from Helius
 */
export async function fetchTransactionDetails(transactionSignature: string): Promise<any> {
  try {
    const response = await fetch(`${HELIUS_API_URL}/transactions/${transactionSignature}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'x-api-key': HELIUS_API_KEY,
    ...
    console.error('Error fetching transaction details from Helius:', error);
    throw error;
  }
}
