 ...
 * @param walletAddress - The wallet address
 * @param scoreData - The score data to update
 * @returns The updated score record
 */
export async function updateWhispererScore(
  walletAddress: string,
  scoreData: Partial<WhispererScoreRecord>
): Promise<WhispererScoreRecord | null> {
  try {
...
/**
 * Gets the top wallets by Whisperer Score
 * @param limit - Maximum number of records to return
 * @returns Array of wallet score records
 */
export async function getTopWhispererScores(
  limit: number = 10
): Promise<WhispererScoreRecord[]> {
  try {
    const { data, error } = await supabase
      .from(WALLET_SCORES_TABLE)
      .select('*')
    ...
    console.error('Error getting top Whisperer Scores:', error);
    return [];
  }
}
